    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="group" whileHover={{ y: -4 }}>
      <Link to={`/product/${product.slug}`}>
        <div className="relative aspect-square overflow-hidden bg-gray-50">
          <motion.img src={product.images?.[0] || product.image} alt={product.name} className="w-full h-full object-cover" whileHover={{ scale: 1.08 }} />
          {product.isNew && <span className="absolute top-3 right-3 bg-luxury-black text-white text-xs px-2 py-1">NEW</span>}
          <button onClick={(e) => { e.preventDefault(); setWishlisted(!wishlisted); }} className="absolute top-3 left-3 p-2 bg-white/90 rounded-full opacity-0 group-hover:opacity-100"><Heart className={`w-4 h-4 ${wishlisted ? 'fill-red-500 text-red-500' : ''}`} /></button>
        </div>
